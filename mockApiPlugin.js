            ...
            server.middlewares.use(function (req, res, next) {
                ...
                req.on('end', function () {
                    ...
                    if (url.startsWith('/api/auth/login') && req.method === 'POST') {
                        var username = body.username || 'godot_dev';
                        var user = {
                            id: Math.floor(Math.random() * 90000) + 10000,
                            username: username,
                            nickname: username === 'godot_dev' ? 'Godot 独立开发者' : username,
                            avatar: "https://avatars.githubusercontent.com/u/".concat((username.length * 12345) % 100000, "?v=4"),
                            bio: '热爱 Godot 游戏引擎与开源技术！',
                            role: username === 'admin' ? 'admin' : 'developer',
                            roleName: username === 'admin' ? '论坛管理员' : '认证开发者',
                            token: "godot_jwt_".concat(Date.now()),
                            joinedAt: '2026-09-21'
                        };
                        res.statusCode = 200;
                    ...
                    if (url.startsWith('/api/auth/register') && req.method === 'POST') {
                        var username = body.username || "user_".concat(Date.now());
                        var nickname = body.nickname || username;
                        var user = {
                            id: Math.floor(Math.random() * 90000) + 10000,
                            username: username,
                            nickname: nickname,
                            avatar: "https://avatars.githubusercontent.com/u/".concat((username.length * 54321) % 100000, "?v=4"),
                            bio: 'Godot 中文论坛新成员。',
                            role: 'member',
                            roleName: '社区成员',
                            token: "godot_jwt_".concat(Date.now()),
                    ...
                    if (url.startsWith('/api/topics/reply') && req.method === 'POST') {
                        var newPost = {
                            ...
                            cooked: "<p>".concat((body.content || '').replace(/\n/g, '<br/>'), "</p>"),
                            post_number: 99,
                            post_type: 1,
