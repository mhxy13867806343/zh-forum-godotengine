...
import { c as joinURL, d as withoutLeadingSlash, f as withoutTrailingSlash, u as withLeadingSlash } from "./_shared-B2PZsp-J.mjs";
import { t as getInternalContext } from "./context-CHpyLX8W.mjs";
import { n as createInteractiveAuth } from "./interactive-auth-BCa_iamv.mjs";
...
function createInstanceShell(options) {
	...
	*/
	function maybeRegister() {
		...
		registerPromise = import("./instance-registry-DtvYxc0Y.mjs").then((n) => n.t).then(({ registerDevframeInstance }) => {
			let port = 0;
			try {
	...
	*/
	...
	let corePromise;
	function ensureCore() {
		corePromise ??= import("./rpc-core-BXyMjbKG.mjs").then((n) => n.n).then(({ createContextRpcServer }) => createContextRpcServer({
			context: ctx,
			auth: resolvedAuth,
