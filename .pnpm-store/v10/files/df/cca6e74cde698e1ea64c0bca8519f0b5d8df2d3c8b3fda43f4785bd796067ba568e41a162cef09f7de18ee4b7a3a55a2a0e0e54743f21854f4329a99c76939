...
import { BetterMap, LAYER_IMPORTS, LAYER_PREFLIGHTS, createGenerator, cssIdRE, toEscapedSelector } from "@unocss/core";
	...
	return [
		{
			...
			async configResolved(config) {
				...
				const envDirs = [];
				for (const env of Object.values(config.environments ?? {})) if (env?.build?.outDir) envDirs.push(resolve(config.root, env.build.outDir));
	...
	function sendUpdate(ids) {
		for (const server of servers) server.ws.send({
			...
			updates: Array.from(ids).flatMap((id) => {
				const mod = server.moduleGraph.getModuleById(id);
				if (!mod) return [];
				return [.../* @__PURE__ */ new Set([mod.url, toViteClientPath(mod.url)])].map((path) => ({
					acceptedPath: path,
					path,
