import { A as splitComma, B as CONTROL_NO_NEGATIVE, C as resolveBreakpoints, E as h, F as globalKeywords, H as SpecialColorKey, I as insetMap, L as positionMap, P as directionMap, R as xyzArray, _ as hyphenate, a as themeTracking, b as numberResolver, d as colorableShadows, g as hasParseableColor, j as cornerMap, k as bracketTypeRe, l as colorCSSGenerator, m as directionSize, n as detectThemeValue, p as defineProperty, r as generateThemeVariable, u as colorResolver, v as isSize, x as parseColor, y as makeGlobalStaticRules, z as xyzMap } from "./utils-C185UPwI.mjs";
import { n as containerParent, r as containerShortcuts, t as container } from "./container-BNtA3wqG.mjs";
...
function* handleColorOrWidth$2(match, ctx) {
	if (isSize(match[1])) yield* handleWidth$2(match, ctx);
	else {
...
function handlerBorderColorOrSize([, a = "", b], ctx) {
	if (a in directionMap) {
		if (isSize(b)) return handlerBorderSize([
			"",
			a,
			b
		], ctx);
...
function handleColorOrWidth$1(match, ctx) {
	if (isSize(match[1])) return handleWidth$1(match, ctx);
...
function handleColorOrWidth(match, ctx) {
	if (isSize(match[1])) return handleWidth(match, ctx);
...
function handlerColorOrSize(match, ctx) {
	if (isSize(match[1])) return handleSize(match, ctx);
